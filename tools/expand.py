...
使い方:
    ...
    python tools/expand.py path/to/main     # path/to/main.rs があれば読む

main.rs 側で使うライブラリを次のどちらかで指定する:
    use cplib::ds::segtree::*;      # 開発時は本物の crate 参照（IDE 補完が効く）
    //@use ds::segtree               # あるいは明示マーカー

main.rs 内に `//@expand` / `// @expand` / `// cplib:expand` のいずれかを
1 行だけ書くと、その位置にライブラリを inline する。予約コメントがなければ
従来通りファイル先頭へ inline する。
...
EXPAND_MARKER_RE = re.compile(
    r"^[ \t]*(?://[ \t]*@expand\b|//[ \t]*cplib:expand\b|/\*[ \t]*@expand[ \t]*\*/)[ \t]*$",
    re.M,
)


def resolve_input_path(path):
    """入力パス。拡張子なしで渡されたら .rs も試す。"""
    if os.path.exists(path):
        return path
    if not path.endswith(".rs") and os.path.exists(path + ".rs"):
        return path + ".rs"
    sys.exit(f"[expand] 入力ファイルが見つかりません: {path}")
...
def main():
    ...
    main_path = resolve_input_path(args.main)
    ...
    library_block = header
    if modblock:
        library_block += modblock + "\n"
    ...
    for a in inner_attrs:
        out += a.strip() + "\n"
    markers = list(EXPAND_MARKER_RE.finditer(user))
    if len(markers) > 1:
        sys.exit("[expand] 展開位置マーカーは 1 つだけにしてください")
    if markers:
        user = EXPAND_MARKER_RE.sub(library_block.rstrip("\n"), user, count=1)
        out += user
    else:
        out += library_block + "\n"
        out += user
